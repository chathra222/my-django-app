...
from users.views import add_user, update_user, delete_user, list_users, get_user
...
class TestUsersViews:
    ...
    def test_delete_user_invalid_method(self):
        client = Client()
        response = client.post(reverse("delete_user", args=["testuser"]))
    ...
    def test_get_user_invalid_method(self):
        client = Client()
        response = client.post(reverse("get_user", args=["testuser"]))
        assert response.status_code == 405
        assert response.json() == {"error": "Invalid method"}